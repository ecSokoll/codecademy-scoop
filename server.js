//Bonus: YAML include appropriate JavaScript modules to read and write
const yamlRead = require('read-yaml');
const yamlWrite = require('write-yaml');
const yamlDatabaseFile = '.database.yml';
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body &&
    ...
    //push commentId to user
    database.users[comment.username].commentIds.push(comment.id);
    //push commentId to article
...
function deleteComment(url, request) {
  ...
  if (!id || !savedComment) {
    ...
    /*delete comment from article
      tests can be passed with to methods:
      1. -> database.articles[savedComment.articleId].commentIds.splice(savedComment.id - 1,1);
      2. -> database.articles[savedComment.articleId].commentIds.splice(database.articles[savedComment.articleId].commentIds.indexOf(savedComment.id),1);

      option 2. seems to be saver in case array of commentIds @ article would be manipulated in database let modus
    */
    ...
    /*delete comment from user
      tests can be passed with to methods:
      1. -> database.users[savedComment.username].commentIds.splice(savedComment.id - 1,1);
      2. -> database.users[savedComment.username].commentIds.splice(database.users[savedComment.username].commentIds.indexOf(savedComment.id),1);

      option 2. seems to be saver in case array of commentIds @ user would be manipulated in database let modus
    */
...
//votes a comment up, using existing fun upvote
...
//votes a comment down, using existing fun downvote
...
//Bonus: load and save data from/to yamlDatabaseFile
function saveDatabase() {
  yamlWrite(yamlDatabaseFile, database, function(err) {
...
function loadDatabase() {
  ...
  if (fs.existsSync(yamlDatabaseFile)) {
    yamlRead(yamlDatabaseFile, function(err, data) {
      if (err){
        throw err
      }else{
        return data;
